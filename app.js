...
export function set_stat_Play_board(played,tail) {
    play_board[tail]=played;
}

export function get_stat_Play_board(){
    return play_board
}
export function get_board_full(){
    return board_full;
}
...
export function check_line(a, b,c){
    let status =
...
export const checkBoardComplete = () => {
    let flag = true;
...
export const addComputerMove = (ai_level) => {
   // set_stat_Play_board(2,computer);
    ...
    if(!board_full){
       ...
       // console.log("pc move" + nextMove);
        //print("pc move" + nextMove);

        play_board[nextMove] = computer;

        game_loop();
    }
    console.log("play board" + play_board);
}
